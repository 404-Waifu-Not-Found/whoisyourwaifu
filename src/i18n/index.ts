...
export const labels = {
  en: {
    ...
    algorithmSections: [
      ...
      },
      {
        ...
        body: 'A power curve maps alignment to fit% so adjacent matches have visible gaps (top match clearly outpaces #2 to #5). Low-confidence runs are hard-capped before display, so an indecisive sheet cannot accidentally claim 95%.',
      },
    ],
    ...
    algorithmFitFormula: 'fit = ((alignment / 0.78 + 1) / 2)^1.4 · 100   where alignment = (1/4) · Σ uᵢ · cᵢ',
    ...
    algorithmFitNote: 'Fit ignores per-axis confidence on purpose: it is the alignment of your vector with the candidate\'s, on a power curve so 1st place visibly outpaces 2nd–5th. The displayed fit is then capped by overall confidence, so an indecisive sheet stays in a believable range.',
  ...
  zh: {
    ...
    algorithmSections: [
      ...
      {
        title: '拉开梯度，不再扁平',
        body: '匹配度会经过幂函数曲线，让相邻名次有可见差距（第一明显甩开 2–5 名）。低置信度的答卷会在显示前被硬性封顶，避免一份模糊答卷误显示 95%。',
      },
    ],
    ...
    algorithmFitFormula: '匹配度 = ((对齐度 / 0.78 + 1) / 2)^1.4 · 100   其中 对齐度 = (1/4) · Σ uᵢ · cᵢ',
    ...
    algorithmFitNote: '匹配度刻意不参考逐轴置信度：它是你的向量与候选向量的对齐度，再经过幂函数曲线，让第一名明显甩开 2–5 名。最终显示的匹配度还会被总体置信度封顶，所以模糊答卷不会冲到不合理的高分区间。',
