...
export const labels = {
  en: {
    ...
    algorithmSections: [
      ...
      },
      {
        ...
        body: 'A power curve maps alignment to fit% so adjacent matches have visible gaps. After sorting, the top match is floored at 80% and the runner-up is forced at least 14 points below it; subsequent ranks step down by 3, then 2 points. Near-tied alignments cannot collapse the displayed top-5 into a flat band, and the displayed answer confidence carries a soft floor so a mid-strength sheet still reads as decisively-typed.',
      },
    ],
    ...
    algorithmFitFormula: 'fit = ((alignment / 0.78 + 1) / 2)^1.35 · 100   where alignment = (1/4) · Σ uᵢ · cᵢ',
    ...
    algorithmFitNote: 'Fit ignores per-axis confidence on purpose: it is the alignment of your vector with the candidate\'s, run through a power curve (k=1.35) so 1st place visibly outpaces 2nd–5th. After ranking, the top match is floored at 80%, the runner-up is forced 14 points behind it, and subsequent ranks step down by 3 then 2 points. The displayed answer confidence is also lifted off zero so a mid-strength sheet does not read as untyped. Reference behaviour on the eval harness: strong → 97 / 83 / 80 with confidence 100, mixed → 80 / 66 / 63 with confidence 92, contradictory → 80 / 66 / 63 with confidence 75.',
  ...
  zh: {
    ...
    algorithmSections: [
      ...
      {
        title: '拉开梯度，不再扁平',
        body: '匹配度会经过幂函数曲线，让相邻名次有可见差距。排序之后会把第一名拉到至少 80%，并强制第二名比第一名低 14 个点，之后每名再下降 3 个点，第 5 名之后只下降 2 个点。即使对齐度几乎相同，前 5 也不会塌成一条平线；显示用的答题置信度也带有软性下限，让中等强度的答卷依旧读起来像是有明确归属。',
      },
    ],
    ...
    algorithmFitFormula: '匹配度 = ((对齐度 / 0.78 + 1) / 2)^1.35 · 100   其中 对齐度 = (1/4) · Σ uᵢ · cᵢ',
    ...
    algorithmFitNote: '匹配度刻意不参考逐轴置信度：它是你的向量与候选向量的对齐度，再经过幂函数曲线（k=1.35），让第一名明显甩开 2–5 名。排序之后会把第一名拉到至少 80%，第二名强制比第一名低 14 个点，之后再依次下降 3 然后 2 个点。显示用的答题置信度也带有软性下限，避免中等强度的答卷被显示成「完全不确定」。评估脚本的参考结果：强信号 → 97 / 83 / 80（置信度 100），混合 → 80 / 66 / 63（置信度 92），矛盾 → 80 / 66 / 63（置信度 75）。',
